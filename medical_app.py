...
def find_nearby_hospitals(city, state):
    """Enhanced hospital finder with more comprehensive data"""
    
    # Enhanced mock data with more realistic hospital information
    base_hospitals = [
        {
            'name': f'{city} Medical Center',
            'address': f'100 Healthcare Blvd, {city}, {state}',
            'phone': '(555) 100-1000',
            'rating': 4.5,
            'emergency': True,
            'specialties': ['Emergency Medicine', 'Cardiology', 'Neurology', 'Trauma'],
            'beds': 350,
            'lat': 40.7128 + (hash(city) % 1000) / 10000,
            'lng': -74.0060 + (hash(city) % 1000) / 10000
        },
        {
            'name': f'{city} General Hospital',
            'address': f'250 Medical Drive, {city}, {state}',
            'phone': '(555) 200-2000',
            'rating': 4.2,
            'emergency': True,
            'specialties': ['Emergency Medicine', 'Surgery', 'Pediatrics', 'Orthopedics'],
            'beds': 280,
            'lat': 40.7589 + (hash(city + '2') % 1000) / 10000,
            'lng': -73.9851 + (hash(city + '2') % 1000) / 10000
        },
        {
            'name': f'{city} Heart Institute',
            'address': f'75 Cardiac Center Way, {city}, {state}',
            'phone': '(555) 300-3000',
            'rating': 4.8,
            'emergency': False,
            'specialties': ['Cardiology', 'Cardiac Surgery', 'Interventional Cardiology'],
            'beds': 120,
            'lat': 40.7505 + (hash(city + '3') % 1000) / 10000,
            'lng': -73.9934 + (hash(city + '3') % 1000) / 10000
        },
        {
            'name': f'{city} Children\'s Hospital',
            'address': f'500 Pediatric Plaza, {city}, {state}',
            'phone': '(555) 400-4000',
            'rating': 4.6,
            'emergency': True,
            'specialties': ['Pediatric Emergency', 'NICU', 'Pediatric Surgery'],
            'beds': 200,
            'lat': 40.7300 + (hash(city + '4') % 1000) / 10000,
            'lng': -73.9900 + (hash(city + '4') % 1000) / 10000
        },
        {
            'name': f'{city} Urgent Care Center',
            'address': f'25 Quick Care Ave, {city}, {state}',
            'phone': '(555) 500-5000',
            'rating': 4.0,
            'emergency': False,
            'specialties': ['Urgent Care', 'Minor Surgery', 'X-Ray'],
            'beds': 15,
            'lat': 40.7400 + (hash(city + '5') % 1000) / 10000,
            'lng': -74.0100 + (hash(city + '5') % 1000) / 10000
        }
    ]
...
def main():
    ...
    * {
    ...
    # Main application (after login)
        ...
        if st.session_state.page == 'dashboard' or st.session_state.page not in [
            ...
            with st.form("diagnosis_form"):
                ...
                with col1:
                    symptoms = st.text_area(
                        ...
                        placeholder="Example: I've been experiencing chest pain for 2 hours, accompanied by shortness of breath and sweating. The pain is crushing and radiates to my left arm...",
                        height=150
                    )
            ...
            if st.button("🔍 Find Healthcare Facilities", use_container_width=True, type="primary"):
                with st.spinner("🔍 Searching for healthcare facilities..."):
                    ...
                    default_center = city_centers.get(city_key, [20.5937, 78.9629])  # Default: India center
                    if hospitals:
                        avg_lat = sum(h['lat'] for h in hospitals) / len(hospitals)
                        avg_lng = sum(h['lng'] for h in hospitals) / len(hospitals)
                        map_center = [avg_lat, avg_lng]
